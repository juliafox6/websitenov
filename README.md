...
Зачем он нужен? Дело в том, что запуская файл `admin.py` через интерпретатор `Python` в его изначальной папке adminboard, он игнорирует настройки порта и пытается запуститься на тот же пот, что и `main.py`. А если мы хотим параллельно запускать оба интерфейса: и пользовательский, и сервисный, то нам нужно запускать их по одному и тому же адресу - то есть, из корневой папки. Для этого существуте файл `run_admin.py`, и именно с помощью него нужно запускать панель администратора.
